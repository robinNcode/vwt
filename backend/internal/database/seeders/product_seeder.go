...
func SeedProducts(db *gorm.DB) {
	...
	for _, c := range currencies {
		db.FirstOrCreate(&c, models.Currency{Code: c.Code})
	}
	...
	for i := range categories {
		db.FirstOrCreate(&categories[i], models.ProductCategory{Slug: categories[i].Slug})
	}

	// 3. Attribute Groups & Attributes
	capacityGroup := models.AttributeGroup{NameBN: "ধারণক্ষমতা", NameEN: "Capacity", SortOrder: 1}
	db.FirstOrCreate(&capacityGroup, models.AttributeGroup{NameEN: "Capacity"})

	wattAttr := models.Attribute{
	...
	db.FirstOrCreate(&wattAttr, models.Attribute{Slug: "watt"})
	...
	for _, opt := range options {
		db.FirstOrCreate(&opt, models.AttributeOption{AttributeID: opt.AttributeID, ValueEN: opt.ValueEN})
	}
