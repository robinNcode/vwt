...
func SeedCatalog(db *gorm.DB) {
	...
	group := models.AttributeGroup{NameBN: "কারিগরি স্পেসিফিকেশন", NameEN: "Technical Specs", SortOrder: 1}
	db.Where(models.AttributeGroup{NameEN: group.NameEN}).FirstOrCreate(&group)
	...
	for i, a := range attrs {
		db.Where(models.Attribute{Slug: a.Slug}).FirstOrCreate(&attrs[i])
	}

	opts := []models.AttributeOption{
	...
	for _, o := range opts {
		db.Where(models.AttributeOption{ValueEN: o.ValueEN, AttributeID: o.AttributeID}).FirstOrCreate(&o)
	}
	...
	for i, p := range products {
		db.Where(models.Product{SKU: p.SKU}).FirstOrCreate(&products[i])
	}
	...
	for _, img := range images {
		db.Where(models.ProductImage{URL: img.URL}).FirstOrCreate(&img)
	}
	...
	for i, v := range variants {
		db.Where(models.ProductVariant{SKU: v.SKU}).FirstOrCreate(&variants[i])
	}
}
