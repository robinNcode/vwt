...
type OrderService interface {
	ListOrders(status string) ([]model.Order, error)
	GetOrderByID(id uint) (*model.Order, error)
	TrackOrder(number string) (*model.Order, error)
	PlaceOrder(req *model.Order) error
...
func (s *orderService) TrackOrder(number string) (*model.Order, error) {
	return s.repo.GetByNumber(number)
}

func (s *orderService) PlaceOrder(o *model.Order) error {
	if o.OrderNumber == "" {
