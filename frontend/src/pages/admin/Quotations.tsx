    ...
    return (
        <div className="space-y-8 pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="font-sora text-2xl font-extrabold text-slate-900 dark:text-white">Quotations & Proposals</h1>
                    <p className="text-sm text-slate-500 dark:text-[#8A8FA8] mt-1">Generate and manage business estimates for potential clients.</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    <div key={i} className="bg-white dark:bg-[#1A1E29] border border-slate-200 dark:border-white/5 p-6 rounded-2xl shadow-sm transition-all hover:shadow-md">
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-xs font-bold text-slate-400 dark:text-[#4D526A] uppercase tracking-wider mb-1">{stat.label}</p>
            ...
            {/* Search and Filters */}
            <div className="flex items-center gap-4 bg-white dark:bg-[#1A1E29] border border-slate-200 dark:border-white/5 p-4 rounded-2xl">
                <div className="relative flex-1">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-[#4D526A]" size={18} />
                    <input
                        type="text"
                        placeholder="Search quotations, clients..."
                        className="w-full bg-slate-50 dark:bg-[#0D0F14] border-none rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-900 dark:text-white focus:ring-1 focus:ring-[#F5A623] transition-all"
                    />
                </div>
                <button className="px-4 py-2.5 bg-slate-50 dark:bg-[#0D0F14] border border-slate-200 dark:border-white/5 rounded-xl text-sm font-bold text-slate-600 dark:text-[#8A8FA8] hover:bg-slate-100 dark:hover:bg-white/5 transition-all">
                    Filter
                </button>
            </div>

            {/* Quotations List */}
            <div className="bg-white dark:bg-[#1A1E29] border border-slate-200 dark:border-white/5 rounded-3xl overflow-hidden shadow-sm dark:shadow-xl dark:shadow-black/20">
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 dark:bg-[#13161E]/50">
                            <tr>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-[#4D526A] uppercase tracking-wider">Number</th>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-[#4D526A] uppercase tracking-wider">Client Info</th>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-[#4D526A] uppercase tracking-wider">Date</th>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-[#4D526A] uppercase tracking-wider">Amount</th>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-[#4D526A] uppercase tracking-wider">Status</th>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-[#4D526A] uppercase tracking-wider text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                            {quotations.map((q) => (
                                <tr key={q.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center">
                                                <FileText size={18} />
                                            </div>
                                            <span className="text-sm font-bold text-slate-900 dark:text-[#F0F2F7]">{q.quotation_number}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-bold text-slate-900 dark:text-[#F0F2F7]">{q.client_name}</span>
                                            <span className="text-xs text-slate-500 dark:text-[#4D526A]">{q.client_email}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5 text-sm text-slate-500 dark:text-[#8A8FA8] font-medium">{q.date}</td>
                                    <td className="px-6 py-5">
                                        <span className="text-sm font-extrabold text-slate-900 dark:text-[#F0F2F7]">${q.total.toLocaleString()}</span>
                                    </td>
                                    <td className="px-6 py-5">
                                    ...
                                    <td className="px-6 py-5">
                                        <div className="flex items-center justify-end gap-2">
                                            <button className="p-2 rounded-xl hover:bg-white/5 text-slate-400 dark:text-[#4D526A] hover:text-blue-500 dark:hover:text-blue-400 transition-all">
                                                <Eye size={18} />
                                            </button>
                                            <button className="p-2 rounded-xl hover:bg-white/5 text-slate-400 dark:text-[#4D526A] hover:text-green-500 dark:hover:text-[#F5A623] transition-all">
                                                <Printer size={18} />
                                            </button>
                                            <button className="p-2 rounded-xl hover:bg-white/5 text-slate-400 dark:text-[#4D526A] hover:text-red-500 transition-all">
