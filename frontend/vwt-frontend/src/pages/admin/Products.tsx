...
const Products: React.FC = () => {
    ...
    const fetchProducts = async () => {
        setLoading(true);
        try {
            const res = await fetch(`http://localhost:8083/api/v1/products?search=${searchTerm}`);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="font-sora text-2xl font-extrabold text-white">Product Management</h1>
                    <p className="text-sm text-[#8A8FA8] mt-1">Manage your catalog, stock, and pricing models.</p>
                </div>
                <button
            ...
            <div className="bg-[#1A1E29] border border-white/5 p-4 rounded-2xl flex flex-col md:flex-row items-center gap-4">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/5 text-sm font-semibold text-[#8A8FA8] hover:bg-white/10 transition-all">
                        <Filter size={18} />
                        Filter
                    </button>
                    <button className="flex-1 md:flex-none px-4 py-2.5 rounded-xl bg-white/5 border border-white/5 text-sm font-semibold text-[#8A8FA8] hover:bg-white/10 transition-all">
                        Sort
                    </button>
                </div>
            </div>
            ...
            <div className="bg-[#1A1E29] border border-white/5 rounded-2xl overflow-hidden shadow-xl shadow-black/20">
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-[#13161E]/50">
                            <tr>
                                <th className="px-6 py-4 text-[10px] font-bold text-[#4D526A] uppercase tracking-wider">Product Info</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[#4D526A] uppercase tracking-wider">Type</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[#4D526A] uppercase tracking-wider">Status</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[#4D526A] uppercase tracking-wider">Featured</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[#4D526A] uppercase tracking-wider">Created</th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-[#4D526A]">No products found.</td>
                                </tr>
                            ) : products.map((product) => (
                                <tr key={product.id} className="hover:bg-white/[0.02] transition-all group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-[#4D526A] group-hover:text-[#F5A623] transition-all overflow-hidden">
                                                <ImageIcon size={20} />
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-[#F0F2F7]">{product.name_en}</p>
                                                <p className="text-xs text-[#4D526A] mt-0.5">{product.name_bn}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        {product.is_featured ? <Check className="text-emerald-500 mx-auto" size={18} /> : <X className="text-[#4D526A] mx-auto" size={18} />}
                                    </td>
                                    <td className="px-6 py-4">
                                        <p className="text-xs text-[#4D526A]">May 09, 2026</p>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <button
                ...
                <div className="p-4 border-t border-white/5 flex items-center justify-between">
                    <p className="text-xs text-[#4D526A]">Showing {products.length} products</p>
                    <div className="flex items-center gap-2">
                        <button className="p-2 rounded-lg bg-white/5 text-[#4D526A] hover:text-[#F0F2F7] disabled:opacity-50" disabled>
                            <ChevronLeft size={18} />
                        </button>
                        <button className="p-2 rounded-lg bg-[#F5A623]/10 text-[#F5A623] text-xs font-bold px-4 hover:bg-[#F5A623]/20">1</button>
                        <button className="p-2 rounded-lg bg-white/5 text-[#4D526A] hover:text-[#F0F2F7]">
